...
export async function POST(request, { params }) {
  ...
  const { rows: existingRows } = await query(
    `SELECT 1 FROM "Like" WHERE "userId" = $1 AND "postId" = $2`,
    [user.id, params.id]
  );

  if (existingRows[0]) {
    await withTransaction(async (client) => {
      await client.query(`DELETE FROM "Like" WHERE "userId" = $1 AND "postId" = $2`, [user.id, params.id]);
      await client.query(`UPDATE "Post" SET "likesCount" = "likesCount" - 1 WHERE id = $1`, [params.id]);
    });
  ...
  await withTransaction(async (client) => {
    await client.query(
      `INSERT INTO "Like" ("userId", "postId", "createdAt") VALUES ($1, $2, now())`,
      [user.id, params.id]
    );
    await client.query(`UPDATE "Post" SET "likesCount" = "likesCount" + 1 WHERE id = $1`, [params.id]);
  });
